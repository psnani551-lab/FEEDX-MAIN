                                                ...
                                                {issue.attachments && issue.attachments.length > 0 && (
                                                    <div className="flex flex-wrap gap-2 mt-3">
                                                        {issue.attachments.map((url, i) => (
                                                            <a key={i} href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 bg-white border border-slate-200 px-2.5 py-1.5 rounded-lg hover:border-blue-400 hover:bg-blue-50 transition-colors group shadow-sm">
                                                                {url.toLowerCase().includes('video') || url.toLowerCase().includes('.mp4') ? <FileVideo className="h-3.5 w-3.5 text-blue-500" /> : url.toLowerCase().includes('.pdf') ? <FileText className="h-3.5 w-3.5 text-red-500" /> : <Image className="h-3.5 w-3.5 text-emerald-500" />}
                            ...
                            <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-2xl mb-4">
                                ...
                                {issue.attachments && issue.attachments.length > 0 && (
                                    <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-emerald-100/50">
                                        ...
                                        {issue.attachments.map((url, i) => (
                                            <a key={i} href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 bg-white border border-emerald-100 px-2.5 py-1.5 rounded-lg hover:border-blue-400 hover:bg-blue-50 transition-colors group shadow-sm">
                                                {url.toLowerCase().includes('video') || url.toLowerCase().includes('.mp4') ? <FileVideo className="h-3 w-3 text-blue-500" /> : url.toLowerCase().includes('.pdf') ? <FileText className="h-3 w-3 text-red-500" /> : <Image className="h-3 w-3 text-emerald-500" />}
