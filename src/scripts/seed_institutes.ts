...
async function seed() {
    const { data, error } = await supabase.from('institutes').upsert(hardcodedInstitutes, { onConflict: 'code' }).select();
    console.log('Seeding Error:', error);
    const check = await supabase.from('institutes').select('id', { count: 'exact' });
    console.log('Total populated:', check.count);
}
